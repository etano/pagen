    ...
    for i in xrange(0, len(particles)):
        for j in xrange(i, len(particles)):
            ...
            for o in objects:
                ...
                paSubgroup.create_dataset('n_r',data=len(paArray[:,0]))
                paSubgroup.create_dataset(paPrefix+'_r',data=paArray[:,1])
                if o['breakup'] != 2:
                    paArray = loadtxt(paPrefix+'d.'+str(paIndex)+'.r.txt', comments='#')
                    paSubgroup.create_dataset(paPrefix+'_long_r_0',data=[paArray[0,1]])
                    paSubgroup.create_dataset('r_long',data=paArray[1:,0])
                    paSubgroup.create_dataset('n_r_long',data=len(paArray[1:,0]))
                    paSubgroup.create_dataset(paPrefix+'_long_r',data=paArray[1:,1])
                    paArray = loadtxt(paPrefix+'d.'+str(paIndex)+'.k.txt', comments='#')
                    paSubgroup.create_dataset(paPrefix+'_long_k_0',data=[paArray[0,1]])
                    paSubgroup.create_dataset('k',data=paArray[:,0])
                    paSubgroup.create_dataset('n_k',data=len(paArray[:,0]))
                    paSubgroup.create_dataset(paPrefix+'_long_k',data=paArray[:,1])
    
                # Write out off diagonal PA
                if o['type'] != 0:
                    paSubgroup = paGroup.create_group('off_diag')
                    paArray = loadtxt(paPrefix+'s.'+str(paIndex)+'.txt', comments='#')
                    xs = unique(paArray[:,0])
                    ys = unique(paArray[:,1])
                    zs = paArray[:,2].reshape((len(xs),len(ys)))
                    paSubgroup.create_dataset('x',data=xs)
                    paSubgroup.create_dataset('n_x',data=len(xs))
                    paSubgroup.create_dataset('y',data=ys)
                    paSubgroup.create_dataset('n_y',data=len(ys))
                    ...
                    for iOrder in range(1,squarer['nOrder']+1):
                        ...
                        paSubSubgroup.create_dataset('n_r',data=len(paArray[:,0]))
