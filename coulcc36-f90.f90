...
!                                                                      C
!  I.J. Thompson and A.R. Barnett, Sept. 1983                          C
!                                                                      C
...
!  description of complex algorithm    JCP 64 (1986) 490-509           C
!  this version written up       in    CPC 36 (1985) 363-372 ACDP      C
!          revised for f90       in    CPC XX (YYYY) ZZZ-ZZZ           C
!                                                                      C
...
!              except in the following cases:                          C
!              (1) if CF1A fails because X too small or ETA too large  C
!               the F solution  is less accurate if it decreases with  C
!               decreasing lambda (e.g. for lambda.LE.-1 & ETA.NE.0)   C
!              (2) if ETA is large (e.g. >> 50) and X inside the       C
!                turning point, then progressively less accuracy       C
!              (3) errors grow near (but not at) the logarithmic       C
!               singularity in the irregular solution for abs(X) << 1. C
!               If D = distance of ZLMIN from the logarithmic soln.,   C
!                then relative errors in GC will be up to min(D,1/D).  C
!                                                                      C
!              RERR in module CSTEED traces the main roundoff errors.  C
!                                                                      C
!   COULCC is coded for:                                               C
!          double precision at least 12-digits and D+-100 exponents    C
!          extended precision at least 24-digits and D+-100 exponents  C
!          Change these values to your requirements.                   C
!                                                                      C
...
!     ACC8     smallest number with 1+ACC8 .ne.1 in REAL(kind=kreal)   C
...
!     Intrinsic functions :   MIN, MAX, SQRT, REAL, AIMAG, ABS, LOG, EXP,
...
!     for any complex argument 'Z' 
!     to any accuracy preset by a GAMMA0 function call.
!
      use LOGAMMA
      IMPLICIT REAL(kind=kreal)(A-H,O-Z)
